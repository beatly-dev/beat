...
Future<String> generateBeatBuildScript() =>
    logTimedAsync(_log, 'Generating build script', _generateBeatBuildScript);

Future<String> _generateBeatBuildScript() async {
  final info = await findBuildScriptOptions();
  final builders = info.builderApplications;
  print("**Running Beat script generator**");
  final library = Library(
    (b) => b.body.addAll([
      ...
      ).assignFinal('_builders').statement,
      _main()
    ]),
  );
...
Future<bool> _allMigratedToNullSafety(
  PackageGraph packageGraph,
  AssetReader reader,
  ...
  final rootPackageFeatures = FeatureSet.fromEnableFlags2(
    sdkLanguageVersion: rootPackageVersion.asVersion,
    flags: const [],
  );
  ...
  for (final import in imports.toSet()) {
    final id = AssetId.resolve(
      Uri.parse(_buildScriptImport(import)),
...
Expression _applyBuilder(BuilderDefinition definition) {
  ...
  return refer('apply', 'package:build_runner_core/build_runner_core.dart')
    ...
    [
      ...
      literalList(
        [for (var f in definition.builderFactories) refer(f, import)],
      ),

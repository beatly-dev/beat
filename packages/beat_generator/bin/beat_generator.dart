...
import 'src/build_script_generate/build_script_generate.dart';
import 'src/command_runners/command_runner.dart';
...
Future<void> main(List<String> args) async {
  ...
  // explicitly invoked.
  var commandRunner =
      BuildBeatCommandRunner([], await PackageGraph.forThisPackage());
  var localCommands = [
    CleanCommand(),
  ];
  ...
  if (localCommandNames.contains(commandName)) {
    ...
    while ((exitCode = await generateAndRun(
          args,
          generateBuildScript: generateBeatBuildScript,
        )) ==
